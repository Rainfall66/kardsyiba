 ...
 *
 * 数据库: window.KARDSYIBA_CARDS(见 cards.js,1578 张全卡表;由数据流水线 kardsyiba-pipeline 生成)
 */
(function () {
  'use strict';
  ...
  // ---------- 衍生卡开关(与卡池模式相互独立) ----------
  // 衍生牌 = 由卡牌效果生成 / 召唤、**不可收集**的牌,卡库里有 61 张。
  ...
  // ---------- 卡图预览 ----------
  // 卡图是**可选资源**(随仓库发布,但游戏不依赖,见 README)。所以这里的原则是
  // 「有图就显示,没图就安静地什么都不做」:加载失败只把预览收起来并进入 5 秒冷却,
  // 免得在缺图的环境里每次悬停都白发一次 404 请求(冷却结束自动恢复,成功一次即清零)。
