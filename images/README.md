...
此目录存放**卡图**(AVIF),已随仓库发布(线上 GitHub Pages 直接引用本目录)。**游戏本体不依赖卡图**,没有图也能正常游玩。

放进去之后游戏会自动启用两个功能:**结算弹窗显示答案卡图**,以及**鼠标悬停卡名时的卡图预览**。
单张图加载失败只会静默收起该次预览并进入 5 秒冷却,所以缺图既不会显示裂图,也不会反复刷 404。
...
- 体积参考:单张约 16 KB,全量 1576 张约 **25.5 MB**;
...
# 2) 下载游戏卡库需要的 1576 张 AVIF(约 48 MB)
...
> 卡图**已纳入版本控制**(全量约 25.5 MB / 1576 张),线上 GitHub Pages 直接引用本目录。
> 早期版本曾用 `.gitignore` 忽略 `images/*.avif`,结果是**线上试玩版一张图都没有**
> —— 预览、候选预览、结算卡图全部静默降级。换图/下架:改完文件直接提交即可。
> 游戏本体不依赖卡图,图缺失时只是不显示,不影响游玩。
> 版权归游戏官方(1939 Games)所有,仅作粉丝向非商业交流使用。
